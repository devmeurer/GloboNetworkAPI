...
class OptionVipGetGrupoCacheByEVipResource(RestResource):

    log = logging.getLogger('OptionVipGetGrupoCacheByEVipResource')

    def handle_get(self, request, user, *args, **kwargs):
        """Treat requests GET to list all grupo de caches of the Option VIP by Environment Vip.

        URL: environment-vip/get/grupo-cache/<id_evip>
        """
        ...
        try:
            ...
            ovips = OptionVip.get_all_grupo_cache(environment_vip.id)

            ovip_dict = dict()
            ovip_list = []

            for ovip in ovips:
                ovip_dict['grupocache_opt'] = ovip.nome_opcao_txt
                ovip_list.append(ovip_dict)
                ovip_dict = dict()

            return self.response(dumps_networkapi({'grupocache_opt': ovip_list}))
