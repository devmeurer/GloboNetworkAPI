...
class OptionVipGetBalanceamentoByEVipResource(RestResource):

    log = logging.getLogger('OptionVipGetBalanceamentoByEVipResource')

    def handle_get(self, request, user, *args, **kwargs):
        """Treat requests GET to list all balanceamento of the Option VIP by Environment Vip.

        URL: environment-vip/get/balanceamento/<id_evip>
        """
        ...
        try:
            ...
            ovips = OptionVip.get_all_balanceamento(environment_vip.id)

            ovip_dict = dict()
            ovip_list = []

            for ovip in ovips:
                ovip_dict['balanceamento_opt'] = ovip.nome_opcao_txt
                ovip_list.append(ovip_dict)
                ovip_dict = dict()

            return self.response(dumps_networkapi({'balanceamento_opt': ovip_list}))
