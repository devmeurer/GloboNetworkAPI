    ...
    def handle_put(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_boolean_param(active):
                self.log.error(
                    'The active parameter is not a valid value: %s.', active)
                ...
                try:
                    ...
                    if usr.eventlog_set.count() != 0:
                        raise UsuarioHasEventOrGrupoError(
                            None, 'Existe event_log associado ao usuario %s' % user.id)

                    if usr.usuariogrupo_set.count() != 0:
                        raise UsuarioHasEventOrGrupoError(
                            None, 'Existe grupo associado ao usuario %s' % user.id)
                    ...
                    self.log.error('Failed to remove the user.')
                    raise GrupoError(e, 'Failed to remove the user.')
